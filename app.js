...
document.querySelectorAll('.calc-select').forEach(card => card.addEventListener('click', () => { const name = card.dataset.name; document.querySelector('#calculator-title').textContent = name; document.querySelector('#calc-model').textContent = name; document.querySelector('.workbench').scrollIntoView({ behavior:'smooth', block:'start' }); }));
